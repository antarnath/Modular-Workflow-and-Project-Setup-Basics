...
class ModelTrainer:
  def __init__(self):
    ...
    # Configure MLFlow
    mlflow.set_tracking_uri(self.model_train_config.mlflow_uri)
    mlflow.set_experiment(self.model_train_config.experiment_name)
  ...
  def train_model(self, X_train, y_train, X_test, y_test, model_name, model, params):
    try:
      with mlflow.start_run(run_name=f'{model_name}_{self.run_name}') as run:
        logging.info(f'Started training with model {model_name}')
        ...
        logging.info(f'Best parameter for {model_name}: {best_params}')
        ...
        logging.info(f'Completed model training {model_name}')
  ...
  def initiate_model_trainer(self, train_array, test_array):
    try:
      ...
      # Training the model and store results
      model_results = {}
      ...
      os.makedirs(os.path.dirname(self.model_train_config.train_model_file_path), exist_ok=True)
